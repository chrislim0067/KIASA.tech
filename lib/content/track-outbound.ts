...
const onClick = (event: Event): void => {
  ...
  if (href.includes('cal.com/utsubo')) {
    ...
    window.dataLayer.push({ event: 'cal_click', source_url: source, locale: LOCALE });
    return;
  }
  if (href.startsWith('mailto:')) {
    window.dataLayer.push({ event: 'email_click', email_context: emailContext(anchor), locale: LOCALE });
  }
};
